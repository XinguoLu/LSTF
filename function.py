...
def load_data():
    ...
    Graphs_indexes = []
    # 读取基因映射数据
    with open("datasets/gene_list.txt", "r") as inf:
    ...
    gene_inter_pairs = []
    R_11 = zeros(shape=(gene_number, gene_number), dtype=int32)
    with open("datasets/Human_Sls.txt", "r") as inf:
        for line in inf:
            id1, id2,  s = line.rstrip().split("\t")
            gene_inter_pairs.append((gene_id_mapping[id1], gene_id_mapping[id2]))  # 元组保存关联对索引
...
def evalution_all(adj_rec, edges_pos_test, edges_neg_test):
    ...
    outputfile1,outputfile2=None,None
    preds = []
    preds_neg = []
