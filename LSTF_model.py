...
class LSTF_model:
    ...
    def __init__(self, ranks=[], nn_size=100, lamda_g=0,lamda_s=0,lamda_gs=[],max_iter=500,numbers=[],theta=0.01,outputfile=None,figdir=None):
        self.ranks=ranks
    ...
    def compute_loss(self):
        ...
        for n in range(0,len(self.Rs)):
            ...
            total_error+=error
            if n==0:
                pos_test_indexes=self.inter_pairs[self.pos_test, :]
                random.seed(123)
                neg_test_indexes=self.noninter_pairs[self.neg_test[random.randint(0, len(self.neg_test), len(pos_test_indexes))], :]
                neg_test_errors=(R_ij_r[neg_test_indexes[:,0],neg_test_indexes[:,1]]**2).sum()
                pos_test_errors=((R_ij_r[pos_test_indexes[:,0],pos_test_indexes[:,1]]-1)**2).sum()
                test_error = neg_test_errors+pos_test_errors
    ...
    def train(self,Rs,Rs_indexes,Graphs,Graphs_indexes=[],inter_pairs=None,noninter_pairs=None,pos_test=None,neg_test=None,Kfold=1):
        ...
        self.Laps=[]
        self.inter_pairs,self.noninter_pairs,self.pos_test,self.neg_test=inter_pairs,noninter_pairs,pos_test,neg_test
        ...
        last_log,last_test_log = self.compute_loss()
        ...
        for step in range(self.max_iter):
            ...
            curr_log,curr_test_log= self.compute_loss()

            delta_log = abs(curr_log - last_log) / abs(last_log)
            delta_test_log=abs(curr_test_log-last_test_log)/abs(last_test_log)
            print("iter:%s, curr_loss:%s, last_loss:%s, delta_loss:%s" % (step, curr_log, last_log, delta_log))
            print(f'curr_test_loss:{curr_test_log}, last_test_loss:{last_test_log}, delta_test_loss:{delta_test_log}')
            train_losses.append(curr_log)
            test_losses.append(curr_test_log)
            ...
            if abs(delta_log) < 1e-5 and step>=300:
                break
            last_log,last_test_log = curr_log,curr_test_log
    ...
    def __str__(self):
        return "Model:  ranks:%s, nn_size:%s, theta:%s, lamda_g:%s, lamda_s:%s,lamda_gs:%s,max_iter:%s" % (self.ranks, self.nn_size,self.theta, self.lamda_g, self.lamda_s,self.lamda_gs, self.max_iter)
